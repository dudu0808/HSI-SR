...
class RGBGuideExtractor(nn.Module):
    ...
    def forward(self, guide_rgb):
        return self.conv_layers(guide_rgb)  # B×out_channels×H×W


class RGBToHSIResidual(nn.Module):
    """RGB -> HSI 残差模块（共享参数，适配任意分辨率），输出通道数=out_channels（=HSI bands）"""
    ...
    def forward(self, guide_rgb):
        return self.conv_layers(guide_rgb)  # B×C×H×W


class SimpleFeaturePyramid(nn.Module):
    """
    简化的特征金字塔（U-Net 风格），支持返回两个尺度特征
    输入通道：in_channels（HSI bands）
    输出：
      - dec1: base_channels × H × W
      - dec2: (2*base_channels) × (H/2) × (W/2)
    """
    ...
    def forward(self, x, return_multi=False):
        """
        x: B×C×H×W
        return_multi=False -> return dec1: B×base×H×W
        return_multi=True  -> return (dec1, dec2)
            dec1: B×base×H×W
            dec2: B×(2base)×H/2×W/2
        """
        # 编码
        enc1 = self.enc1(x)               # B×base×H×W
        enc2 = self.enc2(self.pool(enc1)) # B×2base×H/2×W/2
        enc3 = self.enc3(self.pool(enc2)) # B×4base×H/4×W/4

        # 解码
        dec2_input = torch.cat([self.upsample(enc3), enc2], dim=1)
        dec2 = self.dec2(dec2_input)      # B×2base×H/2×W/2

        dec1_input = torch.cat([self.upsample(dec2), enc1], dim=1)
        dec1 = self.dec1(dec1_input)      # B×base×H×W
...
class FusionHead(nn.Module):
    """
    concat -> 1x1(2base->base) -> SE -> conv(base->out_channels)
    输入：
      hsi_feat:  B×base×H×W
      guide_feat:B×base×H×W
    输出：
      out_res:   B×out_channels×H×W
    """
    ...
    def forward(self, hsi_feat, guide_feat):
        fused = torch.cat([hsi_feat, guide_feat], dim=1)  # B×(2base)×H×W
        fused = self.fuse_1x1(fused)                      # B×base×H×W
        fused = self.attn(fused)                          # B×base×H×W
        out_res = self.final(fused)                       # B×out×H×W
        return out_res
...
class SpectralMixing(nn.Module):
    ...
    """
    def __init__(self, channels: int, spectral_rank: int = 8):
        super().__init__()
...
class LightweightSuperResolutionNet(nn.Module):
    ...
    输入：
      hsi_lr   : B×C×h×w
      guide_rgb: B×3×(h*4)×(w*4)
    输出：
      final_x4 : B×C×(h*4)×(w*4)
      (optional) pred_x2 : B×C×(h*2)×(w*2)
    """
    def __init__(self, in_channels=31, guide_channels=3, base_channels=32, spectral_rank=8):
        ...
        # 把 dec2 的 2base 通道压到 base（匹配 guide_feat）
        self.dec2_reduce = nn.Conv2d(base_channels * 2, base_channels, kernel_size=1)

        # 两个尺度的 head（输出 = C）
        self.head_x2 = FusionHead(base_channels=base_channels, out_channels=in_channels)
        self.head_x4 = FusionHead(base_channels=base_channels, out_channels=in_channels)
    ...
    def forward(self, hsi_lr, guide_rgb, return_x2=False):
        ...
        Inputs:
          hsi_lr   : B×C×h×w  (0~1)
          guide_rgb: B×3×(4h)×(4w)  (0~1)
        Outputs:
          final_x4 : B×C×(4h)×(4w)
          (optional) pred_x2 : B×C×(2h)×(2w)
        """
        # ----- identities -----
        identity_x2 = self.up_x2(hsi_lr)  # B×C×2h×2w
        identity_x4 = self.up_x4(hsi_lr)  # B×C×4h×4w

        # ----- guide feats -----
        guide_feat_hr = self.guide_extractor(guide_rgb)  # B×base×4h×4w
        guide_feat_x2 = F.interpolate(
            guide_feat_hr, scale_factor=0.5, mode="bilinear", align_corners=False
        )  # B×base×2h×2w

        # ----- HSI feats (multi-scale) -----
        hsi_feat_hr, hsi_feat_x2_raw = self.feature_pyramid(identity_x4, return_multi=True)
        hsi_feat_x2 = self.dec2_reduce(hsi_feat_x2_raw)  # B×base×2h×2w

        # ----- ×2 branch -----
        out_res_x2 = self.head_x2(hsi_feat_x2, guide_feat_x2)  # B×C×2h×2w
        guide_rgb_x2 = F.interpolate(guide_rgb, scale_factor=0.5, mode="bilinear", align_corners=False)
        rgb_res_x2 = self.rgb_to_hsi(guide_rgb_x2)             # B×C×2h×2w
        pred_x2 = out_res_x2 + identity_x2 + rgb_res_x2        # B×C×2h×2w
        pred_x2 = self.spectral_mix(pred_x2)

        # ----- ×4 branch -----
        out_res_x4 = self.head_x4(hsi_feat_hr, guide_feat_hr)  # B×C×4h×4w
        rgb_res_x4 = self.rgb_to_hsi(guide_rgb)                # B×C×4h×4w
        pred_x4 = out_res_x4 + identity_x4 + rgb_res_x4        # B×C×4h×4w
        pred_x4 = self.spectral_mix(pred_x4)

        # ----- fusion (final) -----
        pred_x2_up = F.interpolate(pred_x2, scale_factor=2, mode="bilinear", align_corners=False)  # B×C×4h×4w
        refine = self.fuse_refine(torch.cat([pred_x4, pred_x2_up], dim=1))                          # B×C×4h×4w
...
# Generic Test
# =========================
def test_network(in_channels, lr_hw=16, scale=4, batch=2, device=None):
    """
    Generic test:
      LR HSI:    [B, C, lr_hw, lr_hw]
      guide RGB: [B, 3, lr_hw*scale, lr_hw*scale]
      output x4: [B, C, lr_hw*scale, lr_hw*scale]
      output x2: [B, C, lr_hw*2, lr_hw*2]
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"device: {device} | C={in_channels} | lr_hw={lr_hw} | scale={scale}")
    ...
    hsi_lr = torch.randn(batch, in_channels, lr_hw, lr_hw, device=device)
    guide_rgb = torch.randn(batch, 3, lr_hw * scale, lr_hw * scale, device=device)

    with torch.no_grad():
        out = model(hsi_lr, guide_rgb, target_scale="128")
        final_x4, pred_x2 = model(hsi_lr, guide_rgb, target_scale="128", return_x2=True)
    ...
    assert out.shape == (batch, in_channels, lr_hw * scale, lr_hw * scale)
    assert final_x4.shape == (batch, in_channels, lr_hw * scale, lr_hw * scale)
    assert pred_x2.shape == (batch, in_channels, lr_hw * 2, lr_hw * 2)
...
if __name__ == "__main__":
    # ✅ 31-band (CAVE/ICVL/Harvard)
    test_network(in_channels=31, lr_hw=32, scale=4)
    print("-" * 80)

    # ✅ 128-band (Chikusei standard: 16->64)
    test_network(in_channels=128, lr_hw=16, scale=4)
    print("-" * 80)

    # ✅ 103-band (Pavia University common)
    test_network(in_channels=103, lr_hw=32, scale=4)
